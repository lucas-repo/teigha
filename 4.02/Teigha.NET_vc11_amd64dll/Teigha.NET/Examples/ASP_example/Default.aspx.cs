...
namespace ASP_example
{
  public partial class _Default : System.Web.UI.Page
  {
    ...
    {
      using (ExSystemServices Serv = new ExSystemServices())
      {
        using (ExHostAppServices HostApp = new ExHostAppServices())
        {
          TD_Db.odInitialize(Serv);
          String path = AppDomain.CurrentDomain.BaseDirectory;
          String file = path + @"testFile.dwg";
          using (OdDbDatabase pDb = HostApp.readFile(file, true, false, FileShareMode.kShareDenyReadWrite, ""))
          {
            if (pDb != null)
            {
              ...
              TextBox1.Text = "File " + file + " was not open.";
          }
        }
      }
    }
