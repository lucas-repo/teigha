...
/* This is temporary interfaces which will be merged with OdGiSubEntityTraits in future
   binary incompatible release.
   Search for "@@@tmp:#22542" tag in sources to find all temporary changes related to this issue. */
#if (TD_MAJOR_VERSION > 4) || (TD_MINOR_VERSION > 2)
#pragma MARKMESSAGE("4.2 -> 4.3 : AMark : Refactor changes marked by @@@tmp:#22542 tag.");
#endif
...
/** \details
    Temporary protocol extension which provide ability to override OdGsView
    lineweight settings.
    ...
    <group OdGi_Classes> 
*/
class ODGI_EXPORT OdGiLineweightOverridePE : public OdRxObject // @@@tmp:#22542
{
  public:
    ODRX_DECLARE_MEMBERS(OdGiLineweightOverridePE);

    OdGiLineweightOverridePE() {}

    virtual bool pushLineweightOverride(OdGiSubEntityTraits &pTraits, const OdGiLineweightOverride *pOverride) = 0;
    virtual void popLineweightOverride(OdGiSubEntityTraits &pTraits) = 0;
};

typedef OdSmartPtr<OdGiLineweightOverridePE> OdGiLineweightOverridePEPtr;
...
class ODGI_EXPORT OdGiLineweightOverrideHelper
{
  ...
  public:
    ...
    ~OdGiLineweightOverrideHelper()
    {
      if (m_pTraits)
      { // @@@tmp:#22542
        OdGiLineweightOverridePEPtr pLw = OdGiSubEntityTraits::desc()->getX(OdGiLineweightOverridePE::desc());
        if (!pLw.isNull())
          pLw->popLineweightOverride(*m_pTraits);
      }
    }
    void setLineweightOverride(OdGiSubEntityTraits &pTraits, const OdGiLineweightOverride &pOverride)
    {
      ...
      { // @@@tmp:#22542
        m_pTraits = &pTraits;
        pLw->pushLineweightOverride(pTraits, &pOverride);
      }
    }
};
